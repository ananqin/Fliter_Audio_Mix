...
#else
    //经过测试，Mp3，这份代码不能混音MP3，还得命令行。
    FILE *file1 = fopen("buweishui_1m.mp3", "rb");
    if(!file1) {
        printf("fopen buweishui.mp3 failed\n");
        return -1;
    }
    FILE *file2 = fopen("huiguniang.mp3", "rb");
    if(!file2) {
        printf("fopen huiguniang.mp3 failed\n");
        return -1;
    }
    FILE* file_out = fopen("output.mp3", "wb");
    if(!file_out) {
        printf("fopen output.pcm failed\n");
        return -1;
    }
